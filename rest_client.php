...
if(isset($_GET["action"]) == "get_student"){
    ...
    // Get the specific student dataasdsad//sda
    
    $student_info = file_get_contents('http://localhost/html-test/api.php?action=get_student&id=' . $_GET["id"]);
...
<?php
 ...
 
 {
    ...
    $student_list = file_get_contents('http://localhost/html-test/api.php?action=get_student_list');
...
<a href=<?php echo
"http://localhost/html-test/rest_client.php?action=get_student&id=" . $student["id"] ?>><?php echo $student["name"] ?></a><br />
